...
<script>
	function ubahStatusBina(id) {
		$.ajax({
			url: "<?= site_url('pembinaan/statusGanti') ?>",
			type: "POST",
			data: {
				id: id
			},
	...
	function detailData(id) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var kelurahan = data.kelurahan == null ? '-' : data.kelurahan;

				$.ajax({
					url: 'https://dev.farizdotid.com/api/daerahindonesia/kelurahan/' + data.kelurahan,
					type: 'GET',
					dataType: 'json',
					success: function(data) {
						kelurahan = data.nama;
				...
				` + data.alamat + `
				<br>
				` + kelurahan + ` - ` + data.kecamatan + ` - ` + data.kabupaten + `
				</td>
				</tr>
				<tr>
	...
	$(document).ready(function() {
		...
		$('#table1').DataTable({
			...
			"order": [],
			"ajax": {
				"url": "<?= site_url('pembinaan/get_data_belum_dibina') ?>",
				"type": "POST"
			},
			'aoColumnDefs': [{
				'bSortable': false,
